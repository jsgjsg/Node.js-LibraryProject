...
module.exports = {
    list : function(request,response){
        db.query(`SELECT count(*) as total FROM board`,function(error, nums){
            ...
            db.query(`SELECT * FROM board ORDER BY boardid desc, boardid LIMIT ? OFFSET ?`, [numPerPage, offs],function(error, boards) {
                if(error) {
                    throw error;
                }
                var context = {
    ...
    create_process : function(request,response){
        ...
        request.on('end', function() {
            ...
            db.query(`INSERT INTO board (loginid, name, date, content, title) VALUES(?, ?, ?, ?, ?)`,
    ...
    delete : function(request, response) {
        ...
        db.query('DELETE FROM board WHERE boardid = ?', [bNum], function(error, result) {
            ...
            db.query(`SELECT count(*) as total FROM board`,function(error, nums){
                ...
                if(totalPages < pNum && totalPages != 0) {
